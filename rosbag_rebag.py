...
class ReBagger:
        ...
        with rosbag.Bag(self.output_bagname, 'w') as self.output_bag:
            ...
            for topic, msg, t in self.input_bag.read_messages():
                if re.findall("cam0", topic):
                    ...
                    (msg_out.height, msg_out.width) = img_out.shape
                    msg_out.step = msg_out.width  # only for mono8 images, *3 for rgb
    ...
    def save_frame(self, frame_time=30):
        # scan through output_bag to verify correct storage
        t0 = None
        with rosbag.Bag(self.output_bagname, 'r') as self.output_bag:
            for topic, msg, t in self.output_bag.read_messages():
...
if __name__ == "__main__":
    with ReBagger("test_bags/res/euroc_MH_04_res0.10.bag", "test_bags/res/euroc_MH_04_res0.10.bag") as rb:
        # rb.view_output(10)
        rb.save_frame()
    # src_file = "euroc_MH_04.bag"
    #
    # jobs = []
    #
    # blurs = np.arange(5, 101, 5)
    # for blur in blurs:
    #     out_file = f"test_bags/blur/euroc_MH_04_blur{blur:02d}.bag"
    #     keywords = {"blur": blur}
    ...
    #     out_file = f"test_bags/occ/euroc_MH_04_occ{length:03d}^2.bag"
    #     keywords = {"occ": length}
    #     j = multiprocessing.Process(target=multi_bag, args=(src_file, out_file), kwargs=keywords)
    #     jobs.append(j)
    #
    ...
    #     out_file = f"test_bags/res/euroc_MH_04_res{res_factor:0.2f}.bag"
